...
function nav() {
  ...
  return `<header class="site-header"><a class="brand" href="index.html"><span class="brand-mark"><i></i><i></i><i></i></span><span>my <span>body</span></span></a><nav>${links.map(([id, href, label]) => `<a class="${page === id ? "active" : ""}" href="${href}">${label}</a>`).join("")}</nav><span class="header-note"><i></i> 매일 한 걸음</span></header>`;
}
...
function setup() {
  app.innerHTML = `<div class="page-shell">${nav()}<main>${page === "home" ? home() : page === "score" ? score() : recommendations()}</main><footer>My Body · 내 몸을 이해하는 가장 가벼운 시작</footer></div>`;
  if (page === "score") {
